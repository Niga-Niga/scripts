getgenv().Config = getgenv().Config or {
    ["Auto Game"] = {
        ...
        ["Name"] = {
            ["Enable"] = true,
            ["Name Brainrot"] = {"Secret Lucky Block"},
        }
    },
    ["Max slot"] = 10,
    ["Hold Buffer"] = 1,
...
local function isDesired(name, rarity)
    if CFG["Auto Game"]["Name"]["Enable"] and nameMatch(name) then return true end
    if CFG["Auto Game"]["Rarity"]["Enable"] and rarityMatch(rarity) then return true end
    return false
end

local function underHeightLimitY(y)
    return (typeof(y) == "number") and (y <= HEIGHT_LIMIT)
end
...
local function scanOwned(plot)
    ...
    return occupiedAll, list
end

local function countDesiredOwnedUnderLimit(plot)
    local _, list = scanOwned(plot)
    local c = 0
    for _,i in ipairs(list) do
        if underHeightLimitY(i.Y) and isDesired(i.Name, i.Rarity) then
            c += 1
        end
    end
    return c
...
local function scanPods(plot)
    ...
    for _,s in ipairs(pods:GetChildren()) do
        ...
        if not underHeightLimitY(spawn.Position.Y) then
            continue
        end
...
local function scanModels(plot)
    ...
    for _,d in ipairs(plot:GetDescendants()) do
        if d:IsA("Model") and nameMatch(d.Name) then
            ...
            if pos and underHeightLimitY(pos.Y) then
...
local function hasDesiredInMyBase_UnderLimit()
...
getgenv().__AUTO_STEAL_LOCK_ROLE__ = getgenv().__AUTO_STEAL_LOCK_ROLE__ or {}
local roleKey = "uid_" .. tostring(lp.UserId)
if getgenv().__AUTO_STEAL_LOCK_ROLE__[roleKey] == nil then
    getgenv().__AUTO_STEAL_LOCK_ROLE__[roleKey] = (not hasDesiredInMyBase_UnderLimit())
end
local ROLE_IS_MAIN = getgenv().__AUTO_STEAL_LOCK_ROLE__[roleKey]

if not ROLE_IS_MAIN then
    while true do
        if not hasDesiredInMyBase_UnderLimit() then
...
while true do
    local desiredOwned = countDesiredOwnedUnderLimit(myPlot)
    if desiredOwned >= (CFG["Max slot"] or 10) then
    ...
    if target.Position and target.Position.Y and target.Position.Y > HEIGHT_LIMIT then
        skipTarget(target)
        task.wait(0.2)
        continue
    end
