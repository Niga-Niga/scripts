...
do
    ...
    while tick() - start < 30 do
        if workspace:FindFirstChild("Plots") then break end
        task.wait(0.1)
    end
end
...
local function findTargetItem()
    ...
    for _, plot in ipairs(plots:GetChildren()) do
        for _, obj in ipairs(plot:GetChildren()) do
            if enabled[obj.Name] and not collectedItems[obj] then
                local itemPos
