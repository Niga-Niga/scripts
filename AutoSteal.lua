getgenv().Config = getgenv().Config or {
    ["Auto Game"] = {
        ...
        ["Name"] = {
            ["Enable"] = true,
            ["Name Brainrot"] = {"Noobini Pizzanini","Lucky Block","Festive Lucky Block","Admin Lucky Block"},
        }
    },
    ["Max slot"] = 10,
    ["Hold Buffer"] = 1,
...
local function walkTo(pos)
    local tStart = now()
    local path, wps = computePath(hrp.Position, pos)
    if not wps then return false end

    local blocked = false
    local blockConn
    blockConn = path.Blocked:Connect(function()
        blocked = true
    end)
    ...
    for _, wp in ipairs(wps) do
        ...
        local spd = hum.WalkSpeed > 0 and hum.WalkSpeed or 16
        local perTimeout = math.clamp((dist / spd) + 1.5, 2, 10)

        local ok = moveToWait(wp.Position, perTimeout)
        if not ok then
            if blockConn then blockConn:Disconnect() end
            return false
        end

        if now() - tStart > 60 then
            if blockConn then blockConn:Disconnect() end
            return false
        end
    end
