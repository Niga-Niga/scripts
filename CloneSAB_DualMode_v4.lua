...
local function resolveNearbyStealPrompt(expectedName)
    local radius = CFG["Prompt Search Radius"] or 14
    local best, bestDist
    for _,inst in ipairs(workspace:GetDescendants()) do
        if inst:IsA("ProximityPrompt") and inst.Enabled and inst.ActionText == "Steal" then
            local parentPart = inst.Parent and inst.Parent.Parent
            local part = parentPart and parentPart:IsA("BasePart") and parentPart or inst.Parent
            if part and part:IsA("BasePart") then
                local dist = (hrp.Position - part.Position).Magnitude
                if dist <= radius then
                    local okName = true
                    if expectedName and inst.ObjectText and inst.ObjectText ~= "" then
                        okName = (inst.ObjectText == expectedName)
                    end
...
local function holdTarget(info)
    ...
    local p = resolveNearbyStealPrompt(info.Name)
    if p then
